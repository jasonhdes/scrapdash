...
3. **Backend:** dentro de `backend/`, rode `composer install`, copie `.env.example` para `.env` (ajuste `DB_DATABASE=scrapdash` e demais credenciais), crie o banco `scrapdash` no MySQL e rode `php artisan key:generate` seguido de `php artisan migrate`.
